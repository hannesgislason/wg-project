---
title: "Plot code file -  Plot of _ROH_ Figure 1 for the basic filter"
author:
date:
output:
...
# ggsave(filename = "fig1basicpw125x125.pdf",
#        device = "pdf", width = 12.5, height = 12.5, units = "cm", dpi = 600)

```


```{r, fig.width = 7, fig.asp = 0.85, out.width="80%", fig.align="center", fig.cap = "$ROH$ of the eight merged genomes (autosomes) from the Faroe Islands. $a,b$ Total $ROH=KB/1000$ and average $ROH=KBAVG/1000$ are the total and average length of $ROH$, respectively, in the PLINK --homozyg report for variable minimum segment length --homozyg-kb. This we varied from $100$ to $5000$kb in $100$kb increments. $a$ Boxplots of the total $ROH$ (left axis) and of the inbreeding coefficient $F_{ROH} = KB/L$ (right axis) in which  $L = 2881033.286$kb is the autosome length. The median inbreeding $F_{ROH>5}=0.007$ (black dashed line) is the lowest level of recent inbreeding, which is like the average pedigree inbreeding of $0.0067$ and $0.0081$ estimated for multiple scelerosis patients and controls, respectively, from the Faroese population. The $F_{ROH>0.1}=0.258$  (gray dashed line) is the highest level of both recent and ancient inbreeding, which is like $F_{ROH>0.1}$ for the European population of the 1000GP."}
